...
ULONG LdrHashEntry(UNICODE_STRING UniName, BOOL XorHash)
{
    ...
    pRtlHashUnicodeString(&UniName, TRUE, 0, &ulRes);
...
PLDR_DATA_TABLE_ENTRY2 FindLdrTableEntry(PCWSTR BaseName)
{
    PPEB2 pPeb;
...
PRTL_RB_TREE FindModuleBaseAddressIndex()
{
    ...
    if (!pNode->Red)
    {
        ...
        {
            SIZE_T stRet = pRtlCompareMemory((PVOID) stBegin, (PVOID) &pNode, sizeof(SIZE_T));
...
BOOL AddBaseAddressEntry(PLDR_DATA_TABLE_ENTRY2 pLdrEntry, PVOID lpBaseAddr)
{
...
PLIST_ENTRY FindHashTable()
{
    ...
    do
    {
        pCurrentEntry = CONTAINING_RECORD(pEntry, LDR_DATA_TABLE_ENTRY2, InInitializationOrderLinks);
        pEntry = pEntry->Flink;
        ...
        {
            ULONG ulHash = LdrHashEntry(pCurrentEntry->BaseDllName, TRUE);
            pList = (PLIST_ENTRY)((size_t) pCurrentEntry->HashLinks.Flink - ulHash * sizeof(LIST_ENTRY));
            break;
        }
...
BOOL AddHashTableEntry(PLDR_DATA_TABLE_ENTRY2 pLdrEntry)
{
    PPEB pPeb;
    ...
    InsertTailList(&pPebData->InLoadOrderModuleList, &pLdrEntry->InLoadOrderLinks);
    InsertTailList(&pPebData->InMemoryOrderModuleList, &pLdrEntry->InMemoryOrderLinks);
    InsertTailList(&pPebData->InInitializationOrderModuleList, &pLdrEntry->InInitializationOrderLinks);
...
HMODULE IsModulePresent(LPCWSTR lpwName)
{
    ...
    do
    {
        ...
        // TODO: Make this its own ANSI case-insensitive string compare function
        BOOL match = TRUE;
...
{
    STRING aString = { 0 };
    FILL_STRING(aString, ProcName);

    PVOID FunctionAddress = NULL;
    BOOL ok = LocalLdrGetProcedureAddress(hModule, &aString, 0, &FunctionAddress);
    if (!ok)
    {
        return NULL;
    }
...
BOOL _LocalLdrGetProcedureAddress(HMODULE hLibrary, PANSI_STRING ProcName, WORD Ordinal, PVOID* FunctionAddress)
{
    ...
    pNtHeaders = RVA(PIMAGE_NT_HEADERS, hLibrary, ((PIMAGE_DOS_HEADER)hLibrary)->e_lfanew);
    ...
    for (int i = 0; i < pNtHeaders->FileHeader.NumberOfSections; i++)
    {
        pSecHeader = RVA(PIMAGE_SECTION_HEADER, &pNtHeaders->OptionalHeader, pNtHeaders->FileHeader.SizeOfOptionalHeader + i * IMAGE_SIZEOF_SECTION_HEADER);
        if (my_strncmp(".text", pSecHeader->Name, 6))
        {
            startValidSection = RVA(PVOID, hLibrary, pSecHeader->VirtualAddress);
            endValidSection = RVA(PVOID, startValidSection, pSecHeader->SizeOfRawData);
            break;
        }
    }
    ...
    if (pDataDir->Size)
    {
        pExpDir = RVA(PIMAGE_EXPORT_DIRECTORY, hLibrary, pDataDir->VirtualAddress);
        ...
        for (int i = 0; i < numberOfEntries; i++)
        {
            ...
            {
                // searching by name
                ULONG32* pRVA = RVA(ULONG32*, hLibrary, pExpDir->AddressOfNames + i * 4);
                LPCSTR functionName = RVA(LPCSTR, hLibrary, *pRVA);
                ...
                if (my_strncmp(functionName, ProcName->Buffer, ProcName->Length))
                {
                    // found it
                    found = TRUE;
                    short* pRVA2 = RVA(short*, hLibrary, pExpDir->AddressOfNameOrdinals + i * 2);
            ...
            if (found)
            {
                ULONG32* pFunctionRVA = RVA(ULONG32*, hLibrary, pExpDir->AddressOfFunctions + 4 * (FunctionOrdinal - pExpDir->Base));
                PVOID FunctionPtr = RVA(PVOID, hLibrary, *pFunctionRVA);
                ...
                if (startValidSection > FunctionPtr || FunctionPtr > endValidSection)
                {
                    ...
                    if (lib_length != 0)
                    {
                        ...
                        BOOL ok = FALSE;
                        ok = LocalLdrGetProcedureAddress(lib_addr, &funcname_s, 0, &FunctionPtr);
...
BOOL LinkModuleToPEB(PDARKMODULE pdModule)
{
    ...
    pRtlInitUnicodeString(&FullDllName, pdModule->LocalDLLName);
    pRtlInitUnicodeString(&BaseDllName, pdModule->CrackedDLLName);

    // link the entry to the PEB
    pLdrEntry = (PLDR_DATA_TABLE_ENTRY2) pHeapAlloc(pGetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(LDR_DATA_TABLE_ENTRY2));
    ...
    pLdrEntry->DdagNode = (PLDR_DDAG_NODE) pHeapAlloc(pGetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(LDR_DDAG_NODE));
    ...
    // set the entry point
    pLdrEntry->EntryPoint = RVA(PVOID, pdModule->ModuleBase, pNtHeaders->OptionalHeader.AddressOfEntryPoint);
