...
BOOL ParseFileName(PDARKMODULE pdModule, LPWSTR lpwFileName)
{
...
BOOL ReadFileToBuffer(PDARKMODULE pdModule)
{
    HEAPALLOC pHeapAlloc = (HEAPALLOC) GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), "HeapAlloc");
    ...
    HANDLE hFile = pCreateFileW(pdModule->LocalDLLName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    ...
    pdModule->pbDllData = pVirtualAlloc(NULL, dwSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    ...
    if (!pReadFile(hFile, pdModule->pbDllData, dwSize, &pdModule->dwDllDataLen, NULL))
    {
...
PDARKMODULE DarkLoadLibrary(DWORD dwFlags, LPCWSTR lpwBuffer, LPVOID lpFileBuffer, DWORD dwLen, LPCWSTR lpwName)
{
    HEAPALLOC pHeapAlloc = (HEAPALLOC)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), "HeapAlloc");
    GETPROCESSHEAP pGetProcessHeap = (GETPROCESSHEAP)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), "GetProcessHeap");
...
BOOL ConcealLibrary(PDARKMODULE pdModule, BOOL bConceal)
{
